...
public interface OrderRepository extends JpaRepository<Order, Long> {
    ...
    @Query("SELECT COALESCE(SUM(o.totalTTC), 0) FROM Order o WHERE o.statut IN ('PAID','PROCESSING','SHIPPED','DELIVERED') AND o.dateCommande >= :since")
    ...
    @Query("SELECT COALESCE(SUM(l.prixUnitaire * l.quantite), 0) FROM OrderItem l WHERE l.product.seller.id = :sellerId AND l.order.statut IN ('PAID','PROCESSING','SHIPPED','DELIVERED')")
    java.math.BigDecimal revenueBySeller(Long sellerId);

    @Query("SELECT COALESCE(SUM(l.prixUnitaire * l.quantite), 0) FROM OrderItem l WHERE l.product.seller.id = :sellerId AND l.order.statut IN ('PAID','PROCESSING','SHIPPED','DELIVERED') AND l.order.dateCommande >= :since")
    ...
    @Query("SELECT o.dateCommande, o.totalTTC FROM Order o " +
            "WHERE o.statut IN ('PAID','PROCESSING','SHIPPED','DELIVERED') AND o.dateCommande >= :since")
    ...
    @Query("SELECT l.order.dateCommande, l.prixUnitaire, l.quantite FROM OrderItem l " +
            "WHERE l.product.seller.id = :sellerId AND l.order.statut IN ('PAID','PROCESSING','SHIPPED','DELIVERED') AND l.order.dateCommande >= :since")
    ...
    @Query("SELECT s.id, s.nomBoutique, COALESCE(SUM(l.prixUnitaire * l.quantite), 0) AS revenue, COUNT(DISTINCT l.order.id) AS nbOrders " +
            ...
            "WHERE l.order.statut IN ('PAID','PROCESSING','SHIPPED','DELIVERED') " +
    ...
    @Query("SELECT COALESCE(AVG(o.totalTTC), 0) FROM Order o WHERE o.statut IN ('PAID','PROCESSING','SHIPPED','DELIVERED')")
    java.math.BigDecimal averageOrderValue();

    @Query("SELECT COALESCE(AVG(l.prixUnitaire * l.quantite), 0) FROM OrderItem l WHERE l.product.seller.id = :sellerId AND l.order.statut IN ('PAID','PROCESSING','SHIPPED','DELIVERED')")
    ...
    @Query("SELECT p.id, p.nom, COALESCE(SUM(l.quantite), 0) AS sold, COALESCE(SUM(l.prixUnitaire * l.quantite), 0) AS revenue FROM OrderItem l JOIN l.product p " +
            "WHERE p.seller.id = :sellerId AND l.order.statut IN ('PAID','PROCESSING','SHIPPED','DELIVERED') " +
